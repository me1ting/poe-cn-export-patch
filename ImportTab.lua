...
You can get this from your web browser's cookies while logged into the Path of Exile website.
	...
	self.controls.customImportItemsCodeIn = new("EditControl", {"TOPLEFT",self.controls.customImportItemsLabel,"BOTTOMLEFT"}, 0, 4, 250, 20, "", nil, "^%w_%-=", nil, function(buf)
		if #buf == 0 then
			self.customImportItemsCodeState = nil
			return
		end
		self.customImportItemsCodeState = "INVALID"
		local json = Inflate(common.base64.decode(buf:gsub("-","+"):gsub("_","/")))
		if not json then
			return
		end
		self.customImportItemsCodeState = "VALID"
		self.customImportItemsCode = json
	end)
	self.controls.customImportItemsCodeState = new("LabelControl", {"LEFT",self.controls.customImportItemsCodeIn,"RIGHT"}, 4, 0, 0, 16)
	...
	self.controls.customImportTreeCodeIn = new("EditControl", {"TOPLEFT",self.controls.customImportTreeLabel,"BOTTOMLEFT"}, 0, 4, 250, 20, "", nil, "^%w_%-=", nil, function(buf)
		...
		self.customImportTreeCodeState = "INVALID"
		local json = Inflate(common.base64.decode(buf:gsub("-","+"):gsub("_","/")))
		if not json then
			return
		end
		self.customImportTreeCodeState = "VALID"
		self.customImportTreeCode = json
	end)
	self.controls.customImportTreeCodeState = new("LabelControl", {"LEFT",self.controls.customImportTreeCodeIn,"RIGHT"}, 4, 0, 0, 16)
	...
	self.controls.customImportGo.enabled = function()
		return self.customImportItemsCodeState == "VALID" and self.customImportTreeCodeState == "VALID"
	end

	-- Build import/export
	self.controls.sectionBuild = new("SectionControl", {"TOPLEFT",self.controls.customImport,"BOTTOMLEFT"}, 0, 18, 600, 200, "Build Sharing")
